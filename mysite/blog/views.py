...
def post_detail(request, pk):
    # post = Post.objects.get(pk=pk) <- 개발할 때
    post = get_object_or_404(Post, pk=pk) # 공개할 때
    # render함수를 이용해 페이지를 연결해주세요.
    # post_detail.html로 연결해주시고, post 변수를 전달합니다.
    return render(request, 'blog/post_detail.html',
                  {'post':post})
    
def post_new(request):
    # request.method는 현재 이 함수를 싫애하기 위해 들어온 요청이
    # post방식이라면 문자열 'POST'를, GET방식이라면 문자열 'GET'
    # 를 출력한다. 따라서 POST 방식인지를 구분부터 해준다.
    if request.method == "POST":
        # form 변수에 빈 칸이 채워진 Form 양식을 받아옴
        form = PostForm(request.POST) # 입력된 자료가 form으로 넘어옴
        # .is_valid()는 form에서 요청한 자료가 모두 기입되었는지
        # 체크하는 메서드, 만약 모두 기입되지 않았다면 False
        # 모두 기입되었다면 True를 반환한다.
        if form.is_valid():
            #.save()는 아무것도 입력을 안하면 DB에 바로
            # 데이터를 적재해버리기 떄문에 우선 form 양식 이외에
            # 작성자, 퍼블리싱 시간을 추가 적재하기 위해 임시로
            # DB에 올리기만 하기 위해 commit=False
            post = form.save(commit=False) # false면 보류
            ...
            post.save()
            # POST 방식으로 들어왔으며, 자료 적재도 완전히 끝나면
            # 쓴글을 확인할 수 있는 상세 페이지로 이동
            # redirect('url패턴', 우아한url변수명= 넣을 자료)
            return redirect('post_detail', pk=post.pk)
    else:
        # get 방식으로 들어오는 경우는 자료를 반영하지 않고
        # 그냥 닷히 post방식 작성을 유도하기 위해 폼으로 
        # 이동시키는 로직 실행
        form = PostForm()
...
def post_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        # 수정을 시작하는 경우 post_edit(수정양식 템플릿)에
        # 기존글에 대한 정보(form)을 함께 넘겨줌
    return render(request, 'blog/post_edit.html',
                {'form':form})
    
# 몇 번 글을 삭제하는지 요청 받기위해 pk를 입력받음
